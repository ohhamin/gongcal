...
- 일정 생성/수정 모달에 선택 입력인 세부내용 textarea를 추가하고 `events.detaile`에 저장하도록 반영했습니다.
...
- DB 컬럼명 `detaile`은 현재 Supabase 스키마 표기를 그대로 따랐습니다.
- 숨김 일정의 실제 title/detaile 데이터 비공개까지 필요하면 Supabase RLS/RPC 응답 마스킹이 추가로 필요합니다.
