...
- 월간 캘린더 상단 제목은 `yyyy mm` 숫자 형식으로 표시하고, 요일 헤더와 좌/우 영역에 분리된 이전/다음 월 이동 버튼 표시
...
Supabase SQL Editor에서 `web/supabase/create_push_tokens.sql`을 실행하면 모바일 FCM 토큰 저장용 `push_tokens` 테이블과 RLS 정책이 생성됩니다.
