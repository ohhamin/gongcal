...
- 이틀 이상 이어지는 일정은 **긴일정**으로 취급
- 월간 캘린더와 리스트 정렬 우선순위:
  1. 초대된 내 긴일정(참석 여부 저장 전)
  2. 내 긴일정(내가 만든 일정 + 초대받고 참석한 일정)
  3. 초대된 내 일반 일정(참석 여부 저장 전)
  4. 내 일반 일정(내가 만든 일정 + 초대받고 참석한 일정)
  5. 타인 긴일정(타인이 참석한 초대 일정 포함)
  6. 타인 일반 일정(타인이 참석한 초대 일정 포함)
- 같은 일정이 여러 경로로 조회될 때 위 우선순위 기준으로 하나만 표시
- 긴일정끼리는 시작일이 빠른 순, 시작일이 같으면 종료일이 늦은 순, 그래도 같으면 제목 오름차순 정렬
- 일반 일정은 우선순위 후 시작시간 오름차순, 제목 오름차순 정렬
- 긴일정은 FullCalendar의 exclusive end 규칙에 맞춰 종료일 다음 날을 end로 전달해 시작일부터 종료일까지 모두 표시
